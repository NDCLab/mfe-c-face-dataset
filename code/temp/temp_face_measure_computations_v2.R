...
input_organized_path <- paste(proje_wd, "derivatives", "psychopy", "temp_csvs", sep ="/", collapse = NULL) # input directory for data files generated by the organizer script!
output_path <- paste(proje_wd, "derivatives", "psychopy", "temp_stat", sep ="/", collapse = NULL) # output directory
...
for (subject in 1:length(raw_datafiles_list)){
  ...
  surp_rt_threshold <- round(0.2 * number_of_faces_in_surp_memory_task)
  ...
  if (number_of_removed_trials_in_the_memory_surp <= surp_rt_threshold){ # Participants who have less than surp_rt_threshold surprise
    ...
    if (mean(as.numeric(flanker_df$current_trial_accuracy)) >= 0.6){
      ...
      if (surprise_df$num_blocks_excluded[1] <= 2){
...
redcapDat <- read.csv(file = "/Users/kihossei/Google Drive/My Drive/My Digital Life/Professional/GitHub_Repos/mfe-c-face-dataset/derivatives/redcap/Memoryforerrorcface_SCRD_2024-06-13_1453.csv")
...
# Subject IDs: 250037 (epepq); 250001 and 250072 (all the state surveys)

matching_rows <- main_df$participant_id == 250072
...
matching_rows <- main_df$participant_id == 250001
main_df$epepq15_scrdTotal_s1_r1_e1[matching_rows] <- "NA"
main_df$postTask_d_s1_r1_e1[matching_rows] <- "NA"
main_df$stai5_scrdS_s1_r1_e1[matching_rows] <- "NA"
main_df$stai5_scrdS_s1_r1_e2[matching_rows] <- "NA"
...
matching_rows <- main_df$participant_id == 250037
main_df$epepq15_scrdTotal_s1_r1_e1[matching_rows] <- "NA"
